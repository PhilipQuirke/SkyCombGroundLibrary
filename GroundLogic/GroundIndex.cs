...
namespace SkyCombGround.GroundLogic
{
    ...
    public class GroundIndex : BaseConstants
    {
        ...
        public BookModelList BookNames;


        // Normal constructor. Creates index if needed.
        public GroundIndex(string groundDirectory, RectangleF targetArea, string theGeoGcs, bool yAxisPositive)
        {
            ...
            if (! BookLoadSave.Exists(IndexFileName))
                CreateIndex();


            // Load the list of books from the datastore that intersects the target area
            BookLoadSave bookStore = new(IndexFileName);
            bookStore.Load(BookNames, targetArea, theGeoGcs, yAxisPositive);
        }


        // Rebuild constructor. Rarely used. Updates index if needed. Slow.
        public GroundIndex(string groundDirectory, bool yAxisPositive)
        {
            GroundDirectory = groundDirectory.TrimEnd('\\'); // Remove any trailing backslash
            BookNames = new();

            RebuildIndex(yAxisPositive);
        }


        // Create an index of all the books in the ground directory.Very slow.
        ...
        { 
            ...
            if (BookNames.Count > 0)
            {
                ...
                BookLoadSave bookStore = new (fullfileName);
        ...
        private void RebuildIndex(bool yAxisPositive)
        {
            // If we can't find the index, create one.
            if (!BookLoadSave.Exists(IndexFileName))
                CreateIndex();
            ...
            {
                // Load all the books from the datastore 
                BookLoadSave bookStore = new(IndexFileName);
                int lastRow = bookStore.Load(BookNames, new(), "", yAxisPositive);
